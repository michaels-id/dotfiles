-- /Users/michaelm/.config/nvim/init.lua
...
-- vim.keymap.set('n', ';', ':')
-- MIM: I prefer using ; for jumping to next character finding with f

-- MIM: ö is not used by default, Germans mostly map it to ":", so no Shift is required to access the Vim-CommandLine
vim.keymap.set('n', 'ö', ':')

-- MIM: I prefer jk as Esc
...
-- MIM: I prefer Control for direct pane navigation without Ctrl-w
...
-- MIM: Use OS Clipboard by default
		...
		config = function()
			...
			vim.api.nvim_create_autocmd('LspAttach', {
				...
				callback = function(ev)
					...
					-- MIM: I prefer opening the definition in a split on the rigth
					vim.keymap.set('n', 'gd', function()
						vim.cmd("vsplit")
						vim.lsp.buf.definition({
							on_list = function(options)
								-- If there is a single match, jump to it directly
								if #options.items == 1 then
									...
									-- If multiple matches, open Neovim's default quickfix list
					...
					-- MIM: I prefer pane navigation with Control, see above.
